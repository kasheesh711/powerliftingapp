import { computeE1RM_Epley } from './stats';
import type {
  BlockData,
  BlockDescriptor,
  BlockRow,
...
function parseDisplayNumberOrNull(value: unknown): number | null {
  if (value === '' || value === null || value === undefined) {
    return null;
  }
...
function canonicalWeekInfo(weekIndex: number | null | undefined, fallbackIndex: number): WeekInfo {
...
function parseWeekInfoFromRaw(rawValue: unknown, fallbackIndex: number): WeekInfo {
...
function chooseNearestHeaderColumn(
  ...
  rightBound: number,
  anchorCol: number,
  predicate: (normalizedValue: string) => boolean
): number {
  let bestCol = -1;
...
function getExerciseValueForRow(data: string[][], rowIndex: number, daySection: DaySectionInternal): string {
...
function rowHasGroupData(data: string[][], rowIndex: number, group: WeekGroupInternal): boolean {
  const cols = [group.setsCol, group.repsCol, group.targetLoadCol, group.actualLoadCol, group.rpeCol, group.e1rmCol];
...
function detectWeekGroupsFromHeaderRow(
  ...
  const groups: WeekGroupInternal[] = [];
  ...
  for (let i = 0; i < actualCols.length; i++) {
    ...
    const group: WeekGroupInternal = {
      daySectionId,
      headerRow: headerRowIndex,
      dayRow: dayRowIndex,
      labelCol: chooseNearestHeaderColumn(row, leftBound, rightBound, actualCol, (norm) => norm === 'label'),
      ...
      setsCol: chooseNearestHeaderColumn(row, leftBound, rightBound, actualCol, (norm) => norm === 'sets'),
      repsCol: chooseNearestHeaderColumn(row, leftBound, rightBound, actualCol, (norm) => norm === 'reps'),
      targetLoadCol: chooseNearestHeaderColumn(
        row,
        leftBound,
        rightBound,
        actualCol,
        (norm) => norm.includes('target') && norm.includes('load')
      ),
      actualLoadCol: actualCol,
      rpeCol: chooseNearestHeaderColumn(row, leftBound, rightBound, actualCol, (norm) => norm.includes('rpe')),
      e1rmCol: chooseNearestHeaderColumn(
        row,
        leftBound,
        rightBound,
        actualCol,
        (norm) => norm.includes('e1rm') && !norm.includes('peak')
      ),
      notesCol: chooseNearestHeaderColumn(row, leftBound, rightBound, actualCol, (norm) => norm.includes('note')),
      weekIndex: i + 1,
...
function detectDaySectionsFromMarkers(data: string[][]): DaySectionInternal[] {
  const sections: DaySectionInternal[] = [];
  const markers: Array<{ dayIndex: number; dayLabel: string; dayRow: number; markerCol: number }> = [];
  ...
  for (let i = 0; i < markers.length; i++) {
    ...
    let headerRow = -1;
    let weekGroups: WeekGroupInternal[] = [];
...
function detectFallbackDaySectionsFromHeaders(data: string[][]): DaySectionInternal[] {
  const sections: DaySectionInternal[] = [];
  const headers: Array<{ row: number; groups: WeekGroupInternal[] }> = [];
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    const groups = headers[i].groups.map((group, weekOffset) => {
      const cloned: WeekGroupInternal = JSON.parse(JSON.stringify(group));
...
function computePeakE1RMsFromRows(rows: BlockRow[]): PeakE1RMs {
  const peaks: PeakE1RMs = {
    squat: null,
    bench: null,
    deadlift: null
  };
  ...
  for (const row of rows) {
    ...
    const e1 = parseDisplayNumberOrNull(row.e1rm);
    if (!e1) {
      continue;
    }

    if (lift.includes('squat')) {
      peaks.squat = Math.max(peaks.squat || 0, e1);
    } else if (lift.includes('bench')) {
      peaks.bench = Math.max(peaks.bench || 0, e1);
    } else if (lift.includes('deadlift')) {
      peaks.deadlift = Math.max(peaks.deadlift || 0, e1);
    }
  }
    ...
    for (let c = 0; c < row.length; c++) {
      ...
      if (lower.includes('recap')) {
        if (lower.includes('squat')) {
          recaps.squat = String(row[c + 1] || data[r + 1]?.[c] || '');
        } else if (lower.includes('bench')) {
          recaps.bench = String(row[c + 1] || data[r + 1]?.[c] || '');
        } else if (lower.includes('deadlift')) {
          recaps.deadlift = String(row[c + 1] || data[r + 1]?.[c] || '');
        } else if (lower.includes('accessory')) {
          recaps.accessory = String(row[c + 1] || data[r + 1]?.[c] || '');
        }
      }

      if (lower.includes('additional things to fix')) {
        recaps.additions = String(row[c + 1] || data[r + 1]?.[c] || '');
      }

      if (lower.includes("coach's note") || lower.includes('coach note') || lower.includes('changes')) {
        recaps.coach = String(row[c + 1] || data[r + 1]?.[c] || '');
      }
    }
  }
...
function toParserGroups(daySections: DaySectionInternal[]): ParserDayGroup[] {
...
export function parseBlockFromVisualGrid(sheetName: string, data: string[][]): BlockData {
  ...
  if (daySections.length > 0) {
    ...
    parserReport.warnings.push('No DAY markers found. Falling back to repeated header autodetection.');
  ...
  for (const section of daySections) {
    ...
    for (let r = section.startRow; r < Math.min(section.endRowExclusive, data.length); r++) {
      ...
      for (let w = 0; w < section.weekGroups.length; w++) {
        ...
        const e1rm = getCellDisplayValue(data, r, group.e1rmCol);
        ...
        let parsedE1rm = parseDisplayNumberOrNull(e1rm);
        ...
        const weekLabel = group.weekLabel || formatWeekLabel(group.weekIndex || w + 1);
        const dayDisplay = dayName ? `${dayLabel} (${dayName})` : dayLabel;

        const hasTargetOrActual = !isDisplayValueEmpty(targetLoad) || !isDisplayValueEmpty(actualLoad);
    ...
    const weekSummaries = Object.values(weekSummaryMap).sort((a, b) => a.weekIndex - b.weekIndex);
  ...
  parserReport.activeDays = daySummaries.filter((day) => day.isActive).length;
  ...
  parserReport.groups = toParserGroups(daySections);
...
export function getAvailableBlocksFromSheetNames(sheetNames: string[]): BlockDescriptor[] {
...
export function parseDbRowsToBlockData(
  blockName: string,
  dbRows: Array<Record<string, unknown>>,
  dbMetaRow?: Record<string, unknown>
): BlockData {
  const recaps = (dbMetaRow?.RecapsJSON ? safeJsonParse(dbMetaRow.RecapsJSON, {}) : {}) as Recaps;
  const peakE1RMs = (dbMetaRow?.PeakE1RMsJSON
    ? safeJsonParse(dbMetaRow.PeakE1RMsJSON, { squat: null, bench: null, deadlift: null })
    : { squat: null, bench: null, deadlift: null }) as PeakE1RMs;
  const parserReport = (dbMetaRow?.ParserReportJSON
    ? safeJsonParse(dbMetaRow.ParserReportJSON, {
        sheetName: blockName,
        groupsFound: 0,
        groups: [],
        warnings: [],
  ...
  const rows: BlockRow[] = dbRows.map((row) => {
    const weekIndex = parseIntOrNull(row.WeekIndex);
    const dayIndex = parseIntOrNull(row.DayIndex);
    const weekLabel = String(row.WeekLabel || (weekIndex ? formatWeekLabel(weekIndex) : ''));
    const dayLabel = String(row.DayLabel || (dayIndex ? formatDayLabel(dayIndex) : ''));
    const daySectionId = String(row.DaySectionId || (dayIndex ? `day${dayIndex}` : ''));
    const weekSectionId = String(
      row.WeekSectionId || (daySectionId && weekIndex ? `${daySectionId}-week${weekIndex}` : '')
    );

    return {
      sheetName: String(row.BlockName || blockName),
      groupLabel: String(row.GroupLabel || ''),
      rowIndex: Number.parseInt(String(row.RowIndex || ''), 10) || String(row.RowIndex || ''),
      exercise: String(row.Exercise || ''),
      sets: String(row.Sets || ''),
      reps: String(row.Reps || ''),
      targetLoadKg: row.TargetLoadKg === '' ? null : (row.TargetLoadKg as string | number | null),
      actualLoadKg: parseNumberOrNull(row.ActualLoadKg),
      actualLoadCell: row.ActualLoadCell ? String(row.ActualLoadCell) : null,
      rpe: row.RPE === null || row.RPE === undefined || row.RPE === '' ? null : String(row.RPE),
      rpeCell: row.RPECell ? String(row.RPECell) : null,
      e1rm: parseNumberOrNull(row.E1RM),
      ...
      dayRowIndex: parseIntOrNull(row.DayRowIndex),
      weekSectionId,
      daySectionId
    };
  });
  ...
  return {
    ...
    recaps: {
      squat: recaps.squat || '',
      bench: recaps.bench || '',
      deadlift: recaps.deadlift || '',
      accessory: recaps.accessory || '',
      additions: recaps.additions || '',
      coach: recaps.coach || ''
    },
    peakE1RMs,
    parserReport
